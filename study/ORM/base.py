...
# 依赖项，用于获取数据库会话
# 对于标准 JSON API（非 StreamingResponse），yield 之后的清理代码
# 在「响应序列化之后、发送给客户端之前」执行，因此自动 commit 是安全的：
#   1. 路由函数执行 → 返回值序列化为 Response 对象
#   2. yield 之后的代码执行（commit / rollback）
#   3. commit 成功 → 正常响应发给客户端
#   4. commit 失败 → 异常传播 → 客户端收到 500 错误
async def get_database():
    async with AsyncSessionLocal() as session:
        try:
            yield session
            await session.commit()  # 自动提交：路由无需手动 commit
        except Exception:
